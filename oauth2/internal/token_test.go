...
func TestRetrieveToken_InParams(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		io.WriteString(w, `{"status": 0, "body": {"access_token": "ACCESS_TOKEN", "token_type": "bearer"}}`)
	}))
	defer ts.Close()
...
func TestRetrieveToken_Extras(t *testing.T) {
	ResetAuthCache()
	const clientID = "client-id"
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got, want := r.FormValue("client_id"), clientID; got != want {
		...
		w.Header().Set("Content-Type", "application/json")
		io.WriteString(w, `{"status": 0, "body": {"access_token": "ACCESS_TOKEN", "token_type": "bearer", "userid": "363"}}`)
	}))
	defer ts.Close()
	token, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleInParams)
	if err != nil {
		t.Errorf("RetrieveToken = %v; want no error", err)
	}

	raw, ok := token.Raw.(map[string]interface{})
	if !ok {
		t.Errorf("unexpected type for raw: %T", raw)
	}

	userid, ok := raw["userid"].(string)
	if !ok || userid != "363" {
